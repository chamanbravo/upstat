                                        ...
                                        if incidents == nil || (incidents.IsPositive != (heartbeat.Status == "green")) {
                                                ...
                                                if err == nil {
                                                        for _, v := range notificationChannels {
                                                                ...
                                                                if v.Provider == "Ntfy" {
                                                                        ntfyMessage := alerts.NewNtfyAlert(heartbeat, monitor)
                                                                        jsonData, err := json.Marshal(ntfyMessage)
                                                                        if err == nil {
                                                                                _, err := http.Post(v.Data.WebhookUrl, "application/json", strings.NewReader(string(jsonData)))
                                                                                if err != nil {
                                                                                        log.Printf("Error when trying to send ntfy notification: %v", err.Error())
                                                                                }
                                                                        } else {
                                                                                log.Printf("Error when trying to convert ntfy message to JSON: %v", err.Error())
                                                                        }
                                                                }

                                                                if v.Provider == "Gotify" {
                                                                        gotifyMessage := alerts.NewGotifyAlert(heartbeat, monitor)
                                                                        jsonData, err := json.Marshal(gotifyMessage)
                                                                        if err == nil {
                                                                                _, err := http.Post(v.Data.WebhookUrl, "application/json", strings.NewReader(string(jsonData)))
                                                                                if err != nil {
                                                                                        log.Printf("Error when trying to send gotify notification: %v", err.Error())
                                                                                }
                                                                        } else {
                                                                                log.Printf("Error when trying to convert gotify message to JSON: %v", err.Error())
                                                                        }
                                                                }
                                                        }
                                                } else {
